...
function horizontal(grid, start_row, end_row, start_col, end_col, cut) {
  var horiDraw= [];
  for (let i = start_row + 1; i <= end_row - 1; i += 3) {
    horiDraw.push(i);
  }
  shuffle(horiDraw);
  ...
  horiDraw = [];
  for (let i = start_col; i <= end_col; i += 3) {
    horiDraw.push(i);
  }
  shuffle(horiDraw);
  if (horiDraw.length === 0) return;
  ...
  buildRecursiveDivision(grid, start_row, rowId - 1, start_col, end_col);
  buildRecursiveDivision(grid, rowId + 1, end_row, start_col, end_col);
}

function vertical(grid, start_row, end_row, start_col, end_col, cut) {
  var verticaldraw = [];
  for (let i = start_col + 1; i <= end_col - 1; i += 3) {
    verticaldraw.push(i);
  }
  ...
  verticaldraw = [];
  for (let i = start_row; i <= end_row; i += 3) {
    verticaldraw.push(i);
  }
  shuffle(verticaldraw);
  if (verticaldraw.length === 0) return;
  ...
  buildRecursiveDivision(grid, start_row, end_row, start_col, colId - 1);
  buildRecursiveDivision(grid, start_row, end_row, colId + 1, end_col);
}
